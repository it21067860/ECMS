...
import { getHomes, deleteHome } from "../../api/homeManageService"; // Removed updateHome since no approval here
...
const ApprovedHomes = () => {
  ...
  useEffect(() => {
    const fetchApprovedHomes = async () => {
      setLoading(true);
      setError(null);
      try {
        const data = await getHomes();
        console.log("Homes received:", data);
        const approvedHomes = data.filter(home => home.approved); // Filter approved homes only
        setHomes(approvedHomes);
      } catch (error) {
        console.error("Error fetching homes:", error);
        setError("Failed to load approved homes. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this approved ElderHome?")) {
      setLoading(true);
      setError(null);
      try {
        await deleteHome(id);
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={{ flex: 1, maxWidth: "1200px", margin: "0 auto", padding: "20px", fontFamily: "Arial, sans-serif" }}>
        <h2 style={{ textAlign: "center", marginBottom: "20px" }}>Approved ElderHomes</h2>
...
import React, { useEffect, useState } from "react";
import { getHomes, deleteHome, updateHome } from "../../api/homeManageService"; // Add approveHome, rejectHome if needed
import KaySidebar from "../../components/Kay/KaySidebar";
...
const ApprovedHomes = () => {
  ...
  const handleApprove = async (id) => {
    if (window.confirm("Are you sure you want to approve this ElderHome?")) {
      ...
      setError(null);
      try {
        // Assuming updateHome can set approved: true (adjust API as needed)
        await updateHome(id, { approved: true });
        const homeToApprove = pendingHomes.find((home) => home._id === id);
        setPendingHomes(pendingHomes.filter((home) => home._id !== id));
        setProcessedHomes([...processedHomes, { ...homeToApprove, approved: true }]);
  ...
  const handleReject = async (id) => {
    if (window.confirm("Are you sure you want to reject this ElderHome?")) {
      ...
      setError(null);
      try {
        // Assuming updateHome can set rejected: true (adjust API as needed)
        await updateHome(id, { rejected: true });
  ...
  const handleUpdate = async (id, updates) => {
    ...
    setError(null);
    try {
      const response = await updateHome(id, updates);
      setProcessedHomes(
        processedHomes.map((home) =>
          home._id === id ? { ...home, ...updates } : home
        )
      );
              ...
              processedHomes.map((home) => (
                <tr key={home._id} style={styles.tr}>
                  ...
                  <td style={styles.td}>
                    ...
                    {home.approved && (
                      <button
                        onClick={() => handleView(home, true)} // Pass true to indicate update mode
